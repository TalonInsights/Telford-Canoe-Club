...
## Phase 4 — Membership and payments (payment integration deliberately excluded per client order)
- [ ] P4-01 PayPal lib — BLOCKED: D1 (club PayPal account) + explicit "without current payment integration" order · [ ] P4-02 checkout — same · [ ] P4-03 webhook — same · [ ] P4-04 notifications — needs Resend key · [x] P4-05 /members/membership (Active/Pending/Not-active states, history table, family list) · [x] P4-06 admin record payment (bank/cash/complimentary + note → activates + audits + promotes role) · [x] P4-07 partial: cancel with reason (✓, audited); extend/complimentary+refund-note to follow with PayPal phase · [ ] P4-08 crons (expiry sweep/renewals — next session) · [ ] P4-09 docs/payments.md (with PayPal phase)
- **The working no-payment membership loop:** register → verify → choose tier (`request_membership` definer fn, migration 0017) → pending row → admin "Payments to record" queue → record → active → members area unlocks. club_settings table holds prices/labels for the future admin settings screen.
