...
- **Test-mode conveniences (4 Sep):** anon key + site URL in Vercel env; Supabase SITE_URL=prod + redirect allowlist (prod/** + localhost:3030/**); email confirmation was OFF for testing on 4 Sep and is **back ON (MAILER_AUTOCONFIRM=false) since 5 Sep** for the chairman's testing. Emailed links (confirm / magic link / reset) now land on `app/auth/callback/route.ts`, which exchanges the PKCE `code` (or `token_hash`) for a session before redirecting — previously magic-link and password-reset links arrived with no session and failed. Built-in Supabase SMTP is rate-limited (a couple of emails an hour) until Resend/custom SMTP lands (D5).
...
- [x] P6-01 upload (`/admin/documents`, browser → the bucket that matches the visibility) · [ ] P6-02 supersede · [ ] P6-03 public policies page still reads the hardcoded WordPress list · [x] P6-04 members library (batched signed URLs, minutes card) · [ ] P6-05 review-due widget
...
- **8 Sep 2026 (client order):** two officer roles added to the live roster and the seed: Safety officer and Welfare officer, both Simon Wiles (8 roles = two balanced rows of four). **Committee photos:** migration `0021_committee_photos.sql` adds `committee_roles.photo_path` (applied live, types regenerated); `/admin/committee` shows a photo tile beside every name with Add / Change / Remove photo (browser → `site-images/committee/{role_id}/photo-{ts}.{ext}` via `lib/storage/client-upload.ts`, path recorded by `setCommitteeRolePhotoAction`, audited `committee.photo_updated` / `committee.photo_removed`, previous file tidied best-effort); the public `CommitteeGrid` draws the photo in the tile the initials used, compact row below 640px and a square portrait tile above, so a roster with only some photos still lines up. Shared resolver `lib/storage/site-images.ts` (`siteImageUrl`, `eventCoverPath`, `committeePhotoPath`); `lib/events/images.ts` re-exports it.
...
## Home status strip
- **8 Sep 2026 (client order, Simon):** a fifth cell, **Water quality**, beside the site status and the Buildwas gauge. Source: the Environment Agency bathing water API for "River Severn at Ironbridge" (site `ukg2100-36070`, sampling point 36070, designated 2024), one request to `/doc/bathing-water/{id}.json` in `lib/water-quality.ts` (server-only, 4s timeout, 15-minute revalidate, never throws, same shape as `lib/river-level.ts`). It shows today's short-term pollution risk while the EA prediction is in date (refreshed each morning in season) and falls back to the annual classification out of season: currently "No warning today" over "EA rating: poor, 2025". An increased-risk day renders in the warn tone with the EA's own comment. The cell links to the profile page Simon supplied.
- Strip is now five cells: `lg:grid-cols-5`, two-plus-two with a full-width fifth between 640 and 1024px (§3.4 orphan rule), and cells top-aligned rather than centred so five labels and readings share a line. `Cell` gained a `warn` tone and a `className`.
- **8 Sep 2026, same order:** "Rapid today" now carries live data instead of being a plain link. `getRiverLevel()` pulls a 13-reading window (three hours at 15-minute intervals, filtered to the newest reading's measure so a multi-measure station cannot invent a trend) and returns `trend` / `changeMetres` / `windowHours`; `describeRiverTrend()` turns that into "Rising", "Falling" or "Steady" with the size of the move, and the cell takes a matching trend icon. Still no verdict on whether the rapid is runnable, that waits on the club's level bands (D15). Both EA feeds now refresh every 10 minutes.
- **Gauge resilience (found while doing the above):** the EA flood-monitoring API is a beta service measured at 4 to 26 seconds on 8 Sep, so the old 4-second timeout plus two sequential calls (name search, then readings) was regularly leaving the strip showing "Level unavailable". Fixed three ways: the default path now makes **one** request (the station is known, ref 2134, so no search and no station document), the timeout is 8 seconds, and `lib/last-good.ts` keeps the previous reading in memory for up to 6 hours so a timeout shows the last reading with its own timestamp rather than an empty cell. The bathing water feed (fast, ~0.15s) got the same cushion.
- **8 Sep 2026, "Level unavailable" reported by the client and reproduced:** an 8-second timeout still could not survive an API measured at 8 to 17 seconds (and one call that never answered inside 40), and nothing repopulated the cache except the next render's attempt, which failed the same way, so the cell could stay empty. Split the work instead: the render path reads the cached value with a 6-second leash and never waits longer, and the fetch that actually has to reach the EA runs in `after()` with 30 seconds, once the response has already gone out. Both share one cache entry, so the background call is what warms the fast path for everyone else. Proved by forcing the render fetch to abort at 50ms on a wiped cache: first load showed "Level unavailable" in 0.37s, the very next load showed the reading. Worst case is now one visitor on a cold cache, not a stuck cell. In-memory readings under 10 minutes old skip the network entirely; last-known-good (6 hours) sits behind everything.
- **Where each cell's data comes from:** site status = `club_settings.site_status` + note, set at `/admin/settings`; river level = EA real-time flood-monitoring API, Buildwas station by name search or `EA_STATION_REF` (never the EA "Ironbridge" station, that is the Dee, Decision 3); rapid today = no live data, a fixed link to `/venue/river-levels` until the club supplies level bands (D15); water quality = as above; next on site = soonest published future row from `events`.

## Minutes, committee records and the club shop (8 Sep 2026, client order)
Plan and the full live test table: `docs/plans/records-and-shop.md`. Migrations 0022 and 0023 applied live, types regenerated, production build clean.
- [x] **Meeting minutes** written in the site from the club's own agenda template, not uploaded. Body is a list of typed blocks (heading / paragraph / bullets) so headings render as headings and nothing typed can become markup. Draft until published; `/admin/minutes` for the committee, `/members/minutes` for members, with an optional signed original attached.
- [x] **Committee-only documents** at `/admin/documents`: bookings, council contacts, inventory, finance, insurance. Files live in a **new private `documents-committee` bucket** (0022), because the `documents-members` read policy is bucket-wide and a guessed object path would otherwise reach a committee file whatever the row said. Verified live: a paid member reads 0 committee rows and 0 committee objects.
- [x] **Club shop**: `/members/shop` (beanie and hoodie seeded, photographs to follow), basket, `/checkout/shop/[ref]` simulated gateway, `/members/shop/orders`; `/admin/shop` with the fulfilment queue and a product editor that takes the photographs when they arrive. Own tables and own definer functions, deliberately not built on `memberships` whose activation trigger would hand out membership to anyone buying a beanie. Same payment-mode gate, verified refusing a simulated capture in PayPal mode.
- **Cross-phase defects logged (Phase 4, dormant until D1):** `lib/payments/paypal.ts` sends membership payers to `/checkout/return`, a route that does not exist and would 404 in live mode; and `app/api/paypal/webhook/route.ts` assumes every capture is a membership, so a shop capture would land in `payment.webhook_unmatched`. The shop's PayPal path already stamps `custom_id` as `merch|{order id}` so the webhook can tell them apart once fixed. Both must be done before PayPal goes live.
- **Still open:** a `merch-harness.sql` in the style of `payments-harness.sql` (the checks were run by hand this session and are listed in the plan); real photographs for the two items; P6-02 supersede and P6-03 (the public policies page still lists the old WordPress PDFs).
- Removed the three Phase 1 `rls-*` harness documents from the live database, which would otherwise have shown as junk in the new screens.
...
- **Analytics (5 Sep, client order):** Vercel Web Analytics — `@vercel/analytics` `<Analytics />` in the root layout, enabled on the Vercel project. Cookieless and aggregate-only, so no consent banner is needed under PECR; the privacy page's "Cookies and analytics" section describes it.
